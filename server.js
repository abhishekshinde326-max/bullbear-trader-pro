      ...
      try {
        ...
        if (data.feeds) {
          Object.entries(data.feeds).forEach(([key, feed]) => {
            ...
            if (!d) return;
            const ltpc = d.ltpc || {};
            const ohlc = (d.marketOHLC?.ohlc || []).find(o => o.interval === '1d') || {};
            const bids = d.marketDepth?.bid || [];
            const asks = d.marketDepth?.ask || [];
            const tB = bids.reduce((a, b) => a + (b.quantity || 0), 0);
            const tA = asks.reduce((a, b) => a + (b.quantity || 0), 0);
            const diff = tB - tA;
            const diffPct = tB + tA > 0 ? ((diff / (tB + tA)) * 100).toFixed(1) : '0.0';
            const ltp = ltpc.ltp || 0;
            const cp = ltpc.cp || 0;
            const existing = marketData[sym] || {};
            marketData[sym] = {
              sym, price: ltp || existing.price || 0,
              ...
              bids: bids.length ? bids.slice(0,5).map(b=>({price:b.price,qty:b.quantity,orders:b.orders})) : (existing.bids||[]),
              asks: asks.length ? asks.slice(0,5).map(a=>({price:a.price,qty:a.quantity,orders:a.orders})) : (existing.asks||[]),
              totalBid: tB || existing.totalBid || 0,
              totalAsk: tA || existing.totalAsk || 0,
